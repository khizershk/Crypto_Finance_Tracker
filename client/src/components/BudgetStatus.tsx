...
export function BudgetStatus() {
  const { budget, isLoading: budgetLoading, getDaysRemaining, calculateBudgetUsage } = useBudget(DEFAULT_USER_ID);
  ...
  const budgetUsage = calculateBudgetUsage(Array.isArray(transactions) ? transactions : []);
  const daysRemaining = getDaysRemaining();
  
  let statusColor = "bg-green-100 text-green-800";
  if (budgetUsage.percentage > 90) {
    statusColor = "bg-red-100 text-red-800";
  } else if (budgetUsage.percentage > 75) {
  ...
  return (
    <div className="bg-white p-5 rounded-lg border border-slate-200 shadow-sm">
      ...
      <div className="mb-2 flex justify-between text-sm font-medium">
        <span className="text-slate-600">{budgetUsage.used.toFixed(2)} {budget.currency} spent</span>
        <span className="text-slate-800">of {budgetUsage.total.toFixed(2)} {budget.currency} budget</span>
      </div>
      <Progress value={budgetUsage.percentage} className="h-2.5" />
